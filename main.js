...
class Sphere {
  ...
  init = () => {
    ...
    for (let i = -90; i <= 90; i += 10) {
      // each step down count: 19
      for (let j = 0; j < 360; j += 10) {
        //each step around count: 37
        tempx.push(
          this.posX + this.r * Math.cos(toRadians(i)) * Math.cos(toRadians(j))
        );
        tempy.push(
          this.posY + this.r * Math.cos(toRadians(i)) * Math.sin(toRadians(j))
        );
...
class Explosion extends Sphere {
  ...
  explosionInit = () => {
    ...
    for (let i = -60; i <= 60; i += 60) {
      // each step down count: 3
      for (let j = 0; j < 360; j += 60) {
        //each step around count: 6
        tempx.push(
          this.posX + this.r * Math.cos(toRadians(i)) * Math.cos(toRadians(j))
        );
        tempy.push(
        ...
        tempx.push(
          this.posX +
            (this.r + this.num / 2) *
              ...
              Math.cos(toRadians(j))
        );
        tempy.push(
          this.posY +
            (this.r + this.num / 2) *
              Math.cos(toRadians(i)) *
              Math.sin(toRadians(j))
        );
        tempz.push(
          this.posZ + (this.r + this.num / 2) * Math.sin(toRadians(i))
        );
      }
    }
    this.x = tempx;
...
const spawnBacteria = () => {
  ...
  for (let i = length; i < length + temp; i++) {
    ...
    bacteriaArr[i].init();
    vertices = vertices.concat(buildVertices(bacteriaArr[i]));
...
const deleteBacteria = (R, G, B) => {
  for (let i = 0; i < bacteriaArr.length; i++) {
    ...
    if (
      ...
      vertices.splice(
        3 * 6 * 3 * explosionArr.length + 18 * 36 * 2 * 3 * 3 * (i + 1),
        18 * 36 * 2 * 3 * 3
      );
      colors.splice(
        3 * 6 * 3 * explosionArr.length + 18 * 36 * 2 * 3 * 3 * (i + 1),
      ...
      indices.splice(
        3 * 6 * explosionArr.length + 18 * 36 * 2 * 3 * (i + 1),
        18 * 36 * 2 * 3
      );
...
const deleteBacteriaByIndex = (index) => {
  ...
  vertices.splice(
    3 * 6 * 3 * explosionArr.length + 18 * 36 * 2 * 3 * 3 * (index + 1),
    18 * 36 * 2 * 3 * 3
  );
  colors.splice(
    3 * 6 * 3 * explosionArr.length + 18 * 36 * 2 * 3 * 3 * (index + 1),
  ...
  indices.splice(
    3 * 6 * explosionArr.length + 18 * 36 * 2 * 3 * (index + 1),
    18 * 36 * 2 * 3
  );
  updateIndices();
};
  ...
  try {
    for (let i = 0; i < bacteriaArr.length - 1; i++) {
      for (let j = 1; j < bacteriaArr.length; j++) {
        ...
        let dist = Math.sqrt(
          Math.pow(bacteriaArr[i].posX - bacteriaArr[j].posX, 2) +
            Math.pow(bacteriaArr[i].posY - bacteriaArr[j].posY, 2) +
            Math.pow(bacteriaArr[i].posZ - bacteriaArr[j].posZ, 2)
        );
        if (dist <= bacteriaArr[i].r + bacteriaArr[j].r) {
          var bacteria1 = bacteriaArr[i];
          var bacteria2 = bacteriaArr[j];

          let angle1 = (bacteria1.angle1 + bacteria2.angle1) / 2;
          let angle2 = (bacteria1.angle2 + bacteria2.angle2) / 2;
          let midR = Math.sqrt(
            Math.pow(bacteria1.r, 2) + Math.pow(bacteria2.r, 2)
          );
          ...
          vertices = vertices.concat(
            buildVertices(bacteriaArr[bacteriaArr.length - 1])
          );
          colors = colors.concat(
            convertToColors(bacteriaArr[bacteriaArr.length - 1])
          );
          indices = indices.concat(
            convertToIndices(bacteriaArr[bacteriaArr.length - 1])
          );
        }
      }
    }
...
var runDemo = function () {
  ...
  var loop = function (time = 0) {
    ...
    if (sumTime1 > 300) {
      vertices.splice(
        3 * 6 * explosionArr.length + 18 * 36 * 2 * 3 * 3,
        18 * 36 * 2 * 3 * 3 * bacteriaArr.length
      );
      for (let i of bacteriaArr) {
        i.r += 0.01;
        i.init();
        vertices = vertices.concat(buildVertices(i));
      }
      collide();
    ...
    if (sumTime3 > 100) {
      //explode doesnt work yet (deletes necessary vertices and doesn't show neseccary bacteria explosion)
      // explode();
      sumTime3 = 0;
    }
